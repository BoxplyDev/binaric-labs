...
function ServiceRow({ index, icon: Icon, title, description, tag, delay, inView }: {
  index: number; icon: React.FC<{ className?: string }>; title: string;
  description: string; tag: string; delay: number; inView: boolean;
}) {
    ...
    <motion.div
      ...
      <div className="relative py-5 md:py-7 grid grid-cols-[32px_1fr_auto] md:grid-cols-[56px_1fr_160px_auto] gap-3 md:gap-8 items-start">
        <motion.span animate={{ opacity: hovered ? 0.5 : 0.18 }} transition={{ duration: 0.25 }}
          className="text-[10px] tracking-[0.25em] text-white pt-0.5">{String(index).padStart(2, '0')}</motion.span>
        <div>
          <div className="flex items-center gap-2.5 mb-0">
            <motion.div
              animate={{ borderColor: hovered ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.07)', backgroundColor: hovered ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.02)' }}
              transition={{ duration: 0.25 }} className="w-5 h-5 md:w-6 md:h-6 border flex items-center justify-center shrink-0">
              <Icon className="w-2.5 h-2.5 md:w-3 md:h-3 text-white/60" />
            </motion.div>
          ...
          <motion.p
            ...
            className="text-white/35 text-xs leading-[1.85] font-light overflow-hidden ml-8" style={{ height: 0 }}>
            {description}
          </motion.p>
        </div>
...
export default function PremiumServices() {
  ...
  const services = [
    { icon: Globe,    title: 'WEB DEVELOPMENT', description: 'Premium websites engineered for performance and conversion. Custom design, e-commerce, SEO-first architecture — built to load fast and close faster.', tag: 'WEB / PLATFORMS'  },
    { icon: Users,    title: 'SOCIAL MEDIA',     description: 'Strategic social management that drives measurable revenue. Content calendars, community velocity, paid amplification — all in one orbit.',           tag: 'SOCIAL / CONTENT' },
    { icon: Megaphone,title: 'ADVERTISING',      description: 'Data-driven campaigns across every surface. Precision targeting, continuous optimisation, and ROI you can read without a decoder ring.',               tag: 'PAID / GROWTH'    },
    { icon: Rocket,   title: 'FULL SERVICE',     description: 'Complete digital transformation — every channel integrated, every signal connected. One team, one strategy, maximum gravitational pull.',               tag: 'STRATEGY / ALL'   },
  ];
    ...
    <section ref={ref} className="relative min-h-screen bg-black overflow-hidden">
      <div className="absolute inset-0">
        ...
        <motion.div style={{ y: y2 }} className="absolute inset-0">
          <div className="absolute top-1/3 left-1/4 w-[400px] h-[400px] bg-gradient-to-br from-white/[0.025] via-transparent to-transparent rounded-full blur-3xl" />
          <div className="absolute bottom-1/4 right-1/3 w-[300px] h-[300px] bg-gradient-to-tl from-white/[0.02] via-transparent to-transparent rounded-full blur-3xl" />
        </motion.div>
          ...
          <motion.div style={{ opacity, scale, y }} className="max-w-6xl mx-auto">
            ...
            <div className="mb-12 md:mb-24">
              <motion.span initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.7 }} className="text-white/20 text-[10px] md:text-sm tracking-[0.3em] block mb-6 md:mb-8">SERVICES</motion.span>
            ...
            <div className="grid lg:grid-cols-2 gap-12 md:gap-24 items-start mb-16 md:mb-24">
              <div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
                  ...
                  {['No retainers built on vanity metrics.', 'Every channel connected. Every signal read.', 'Results measured in revenue, not reach.'].map((line, i) => (
              ...
              <div>
                ...
                <div>
                  {services.map((s, i) => (
                    <ServiceRow key={i} index={i + 1} icon={s.icon} title={s.title}
                      description={s.description} tag={s.tag} delay={0.6 + i * 0.12} inView={inView} />
                  ))}
                </div>
                <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                  ...
                  <motion.button whileHover="hover" whileTap={{ scale: 0.98 }} className="group relative w-full px-6 md:px-8 py-4 md:py-5">
                    ...
                    <div className="relative flex items-center justify-between">
                      <span className="text-white/70 tracking-[0.2em] text-xs uppercase font-light">Explore Services</span>
                      <motion.div variants={{ hover: { x: 5 } }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}
                        ...
                        <motion.span className="text-white/40 text-xs tracking-[0.2em] uppercase font-light"
                          ...
                          style={{ overflow: 'hidden' }}>View</motion.span>
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <span className="text-white/15 text-[9px] tracking-[0.25em] hidden sm:block">ENGINEERED FOR DIGITAL ESCAPE VELOCITY</span>
