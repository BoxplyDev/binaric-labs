...
function ParticleCanvas({ explodeRef }: { explodeRef: React.MutableRefObject<((x: number, y: number) => void) | null> }) {
  ...
  const explode = useCallback((wx: number, wy: number) => {
    ...
    const cx = wx - rect.left;
    const cy = wy - rect.top;
    for (let i = 0; i < 36; i++) {
...
// ─── Single magnetic letter ───────────────────────────────────────────────────
function MagneticLetter({
  char, index, total, dimmed, onExplode,
}: {
  ...
  const onLeave = useCallback(() => {
    setHot(false); rx.set(0); ry.set(0);
  }, [rx, ry]);
  ...
  return (
    ...
    >
      ...
      <span aria-hidden className="absolute inset-0 font-bold tracking-tighter leading-none pointer-events-none"
        style={{ color: hot ? 'rgba(255,180,160,0.18)' : 'transparent', transform: 'translate(-3px,0)', filter: 'blur(1px)', transition: 'color 0.2s' }}>
        {char}
      </span>
      {/* Blue aberration */}
      <span aria-hidden className="absolute inset-0 font-bold tracking-tighter leading-none pointer-events-none"
        style={{ color: hot ? 'rgba(160,200,255,0.18)' : 'transparent', transform: 'translate(3px,0)', filter: 'blur(1px)', transition: 'color 0.2s' }}>
        {char}
      </span>
      {/* Main */}
      <span className="relative font-bold tracking-tighter leading-none transition-all duration-150"
        style={{
          color: dimmed
            ? (hot ? 'rgba(255,255,255,0.55)' : 'rgba(255,255,255,0.15)')
            : (hot ? 'rgba(255,255,255,1)'    : 'rgba(255,255,255,0.92)'),
...
function InteractiveWordmark({ explodeRef }: { explodeRef: React.MutableRefObject<((x: number, y: number) => void) | null> }) {
  ...
  const fs = 'clamp(3rem, 10.5vw, 8rem)';

  return (
    <div ref={ref} className="relative mb-16 md:mb-20 select-none">
      {/* Glitch duplicate layers */}
      {glitch && (
        <>
          <div className="absolute inset-0 pointer-events-none z-10 mix-blend-screen" style={{ transform: `translate(${(Math.random()-0.5)*10}px, 0)`, opacity: 0.3, filter: 'blur(1px)' }}>
          ...
          <div className="absolute inset-0 pointer-events-none z-10 mix-blend-screen" style={{ transform: `translate(${(Math.random()-0.5)*-8}px, 2px)`, opacity: 0.25, filter: 'blur(0.5px)' }}>
      ...
      <motion.div
        ...
        animate={visible ? { opacity: 1, y: 0, skewX: 0 } : {}}
        transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}
        style={{ fontSize: fs, perspective: '1000px' }}
      ...
      <motion.div
        ...
        className="h-px bg-gradient-to-r from-white/15 to-transparent mt-6 max-w-xs"
        style={{ originX: 0 }}
      />
      ...
      <motion.p
        ...
        className="text-[9px] tracking-[0.3em] text-white/12 font-light mt-3"
      >
...
function FooterLink({ label, href, icon: Icon, delay }: { label: string; href: string; icon?: React.ElementType; delay: number }) {
  ...
  return (
    <motion.a
      ...
      className="flex items-center gap-2.5 text-white/30 hover:text-white/70 transition-colors duration-200"
    >
      {Icon && <Icon className="w-3.5 h-3.5 shrink-0" />}
      <span className="text-xs tracking-[0.15em] font-light">{label}</span>
    ...
    <footer className="relative bg-black overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-20 pb-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10 md:gap-16 mb-20">
          <div>
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
              className="text-[9px] tracking-[0.35em] text-white/20 font-light block mb-5">COMPANY</motion.span>
          ...
          <div>
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.08 }}
              className="text-[9px] tracking-[0.35em] text-white/20 font-light block mb-5">LEGAL</motion.span>
            <div className="space-y-3">
          ...
          <div>
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.16 }}
              className="text-[9px] tracking-[0.35em] text-white/20 font-light block mb-5">CONNECT</motion.span>
            <div className="space-y-3">
        ...
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }}
          ...
          className="h-px bg-gradient-to-r from-white/[0.08] via-white/[0.04] to-transparent mb-8"
          style={{ originX: 0 }} />
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <p className="text-white/15 text-[9px] tracking-[0.3em] font-light hidden md:block">
            ENGINEERING DIGITAL GRAVITY
          </p>
          <motion.a href="#" whileHover="hover"
            className="group flex items-center gap-3 text-white/20 hover:text-white/50 transition-colors duration-200">
